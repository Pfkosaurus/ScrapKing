...
public class Paper_room : MonoBehaviour
{
	...
	public int m_copy_paper,m_newspaper,m_packaging,m_contaminated;
	...
	public void Separator()
	{
		...
		if(warehouse.GetComponent<Warehouse>().m_paper > 0 && warehouse.GetComponent<Warehouse>().paper_recycle)
		{
			if(level > 0)
			{
				m_buy = true;
		...
		if(m_buy && economicManager.GetComponent<Economic_manager>().coins > 0)
		{
			...
			if(timer > timeToInvoke)
			{
				...
				go.GetComponent<Rigidbody2D>().velocity = new Vector2 ( 5f , 0f );
				...
				warehouse.GetComponent<Warehouse>().m_paper--;
	...
	void Update()
    {
		...
		if(warehouse.GetComponent<Warehouse>().paper_recycle){ Separator(); }else{ }
    }
}
